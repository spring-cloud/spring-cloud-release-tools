...
public class TurbineStreamApplicationTests {
	...
	public void discoveryClientIsEureka() {
		...
		assertTrue(
				"composite discovery client should be composed of Eureka and Simple Discovery client's",
				compositeDiscoveryClient.getDiscoveryClients().size() == 2);

		assertTrue(
				"the composed discovery client should have a EurekaDiscoveryClient with a higher precedence ",
				compositeDiscoveryClient.getDiscoveryClients()
						.get(0) instanceof EurekaDiscoveryClient);

		assertTrue(
				"the composed discovery client should have a SimpleDiscoveryClient with a lower precedence ",
				compositeDiscoveryClient.getDiscoveryClients()
						.get(1) instanceof SimpleDiscoveryClient);
	}
